...
async function handlePlay() {
  ...
  state = { ...state, spinning: true, stopRequested: false };
  setControlsDisabled(elements, true);
  setRunActive(elements, true);
  ...
  for (let index = 0; index < totalRounds; index += 1) {
    ...
    if (state.stopRequested) {
      setStatus(elements, `Stopped after round ${state.roundsPlayed}.`);
      break;
    }
  ...
  state = { ...state, spinning: false, stopRequested: false };
  setLeverPulling(elements, false);
  setControlsDisabled(elements, !canAffordRound(state.tokens));
  setRunActive(elements, false);
}
...
function handleStop() {
  ...
  setStatus(elements, "This run will stop after the current spin finishes.");
}
